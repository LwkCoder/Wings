 ...
 * 【该类下不应调用PopCreator实现的IPopOpeartor显示方法】
 */
...
public abstract class PopBaseContentView
{
     ...
     */
    public void attach(Context context, PopOptions options, PopCreator popCreator)
    {
        this.mContextReference = new WeakReference<>(context != null ? context : Utils.getContext());
        this.mOptions = options;
        this.mPopCreator = popCreator;
        inflateRealContentView(getContext(), getPopOptions(), getPopCreator());
    }

    //初始化
    private void inflateRealContentView(Context context, PopOptions options, PopCreator popCreator)
    {
        //解决xml根布局定义宽高无效的问题
     ...
     * 获取Pop配置参数
     */
    public PopOptions getPopOptions()
    {
        return mOptions;
    }
